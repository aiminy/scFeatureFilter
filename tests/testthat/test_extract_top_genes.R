...
test_that("Giving the expected result", {
    expect_equal(
        suppressMessages(extract_top_genes(input1, window_size = 2)),
        result1
    )
    expect_equal(
        suppressMessages(extract_top_genes(input1, mean_expression = 4)),
        result1
    )
    expect_equal(
        suppressMessages(extract_top_genes(input1, min_expression = 4)),
        result1
    )
})
...
test_that("Working with ties", {
    expect_equal(
        suppressMessages(extract_top_genes(input1, window_size = 3)),
        result2
    )
})